...
import { fetchTags, saveTag } from "@/lib/api/upload";
...
export default function NamingDialog({
  ...
  const { getTagColorIndex, setTagColorIndex, getTagColor } = useTagColors();
  ...
  const addNewTag = () => {
    ...
    setNewTag("");
    // Persist tag to backend so it appears in future uploads
    saveTag(trimmed).catch(() => {});
  };
                ...
                return (
                  <div key={tag} className="relative">
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium border transition-all duration-150 cursor-pointer ${colors.bg} ${colors.text} ${
                        selected
                          ? `${colors.border} ring-1 ring-offset-1 ring-current`
                    ...
                    >
                      ...
                      <span
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setPickerOpenFor(
                            pickerOpenFor === tag ? null : tag,
                          );
                        }}
                        className={`w-3 h-3 rounded-full shrink-0 transition-transform hover:scale-125 ${colors.swatch}`}
                      />
                    ...
                    {pickerOpenFor === tag && (
                      <TagColorPicker
                        ...
                        onSelect={(i) => setTagColorIndex(tag, i)}
