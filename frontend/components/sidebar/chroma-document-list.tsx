...
export default function ChromaDocumentList({
  ...
  const handleTitleSave = useCallback(
    async (fileId: string) => {
      ...
      // Optimistic update
      setDocs((prev) =>
        prev.map((d) => (d.file_id === fileId ? { ...d, title: trimmed } : d)),
      );
      setEditingId(null);
  ...
  for (const doc of docs) {
    filenameCounts.set(
      doc.filename,
      (filenameCounts.get(doc.filename) ?? 0) + 1,
    );
  }
  ...
  return (
    <div className="px-3 pb-3 space-y-2">
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2">
                <div
                  className={`h-3.5 w-3.5 flex-shrink-0 mt-0.5 rounded-full border-2 border-t-transparent animate-spin
      ...
      <AnimatePresence initial={false}>
        {docs
          .filter((d) => !pendingIngestions.some((p) => p.fileId === d.file_id))
          .map((doc, index) => {
                  ...
                  <div className="flex-shrink-0 flex items-center gap-0.5">
                    {onOpenReader && (
                      <motion.button
                        whileTap={{ scale: 0.9 }}
                        onClick={() =>
                          onOpenReader(doc.filename.replace(/\.pdf$/i, ""))
                        }
