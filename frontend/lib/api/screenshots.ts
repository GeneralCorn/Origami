...
export async function fetchPendingScreenshots(): Promise<PendingScreenshot[]> {
  const response = await fetch(`${API_URL}/api/screenshots/pending`);
  if (!response.ok) {
...
export async function fetchDigestContent(
  ...
  const response = await fetch(`${API_URL}/api/digests/${week}`);
  if (!response.ok) {
...
export async function recategorize(
  ...
  const response = await fetch(`${API_URL}/api/digests/${week}/recategorize`, {
    method: "PATCH",
...
export function screenshotUrl(filename: string): string {
  return `${API_URL}/api/screenshots/${filename}/file`;
}

export async function deleteScreenshot(name: string): Promise<void> {
  const response = await fetch(`${API_URL}/api/screenshots/${name}`, {
    method: "DELETE",
  });
  if (!response.ok) {
