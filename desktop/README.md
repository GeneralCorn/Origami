...
- **There is no way to set `ANTHROPIC_API_KEY` in a packaged build.** `main.py` loads `.env.local` from its own directory, which is now inside a read-only, signed bundle. Chat will not work in a packaged app until configuration moves to `userData` or to a settings surface.
