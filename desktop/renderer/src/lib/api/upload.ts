...
export async function confirmUpload(
  ...
  const response = await apiFetch(`${API_URL}/api/upload/confirm`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function fetchPDFs(): Promise<PersistedPDF[]> {
  const response = await apiFetch(`${API_URL}/api/pdfs`);
  if (!response.ok) {
...
export async function deletePDF(name: string): Promise<void> {
  const response = await apiFetch(`${API_URL}/api/pdfs/${name}`, {
    method: "DELETE",
  });
  if (!response.ok) {
